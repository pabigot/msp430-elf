...
copy_include (char *srcn, char *destn)
{
  ...
  while (fgets (line, 4096, src))
      ...
      if (strstr(destn, "stdlib.h"))
	{
	  ...
	  if (strstr (line, "matherr(struct"))
	    continue;
	  if (strstr (line, "define complex _complex"))
	    continue;
	}

      if (strstr(destn, "math.h"))
	{
	  if (strstr (line, "define exception"))
	    {
	      fprintf (dest, "\n");
	      fprintf (dest, "_CRTIMP int __cdecl _matherr(struct _exception *);\n");
	      continue;
	    }
	}
  ...
  if (strstr(destn, "stdlib.h"))
    {
      fprintf(dest, "#ifndef _PTRDIFF_T_DEFINED\n");
      fprintf(dest, "typedef long int ptrdiff_t;\n");
      fprintf(dest, "#define _PTRDIFF_T_DEFINED\n");
      fprintf(dest, "#endif\n");
      fprintf(dest, "#ifndef BUFSIZ\n");
      fprintf(dest, "#define BUFSIZ 1024\n");
      fprintf(dest, "#endif\n");
    }
  fclose (src);
  fclose (dest);
