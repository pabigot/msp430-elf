...
md_assemble (char * str)
{
  ...
  if (rl78_bytes.link_relax && rl78_bytes.n_fixups)
    {
      fixS * f;
		 ...
		 fragS * fragP ATTRIBUTE_UNUSED)
{
  ...
  tprintf ("convert, op is %d, disp %d (%lx-%lx)\n",
	   rl78_opcode_type (fragP->fr_opcode), disp,
  ...
  if (fragP->fr_next != NULL
	  && ((offsetT) (fragP->fr_next->fr_address - fragP->fr_address)
	      != fragP->fr_fix))
    as_bad (_("bad frag at 0x%p : fix %ld addr 0x%p %ld - 0x%p %ld \n"), fragP,
	    (long) fragP->fr_fix,
	    fragP->fr_next, (long) fragP->fr_next->fr_address,
	    fragP, (long) fragP->fr_address);
}
