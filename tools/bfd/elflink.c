		       ...
		       struct elf_reloc_cookie *cookie)
{
  ...
  if (r_symndx >= cookie->locsymcount
	...
	h = (struct elf_link_hash_entry *) h->root.u.i.link;
      h->mark = 1;
      /* A relocation against an absolute symbol is suspicious,
	 so if the user is interested, let them know.  */
      if (info->print_gc_sections
	  && h->root.type == bfd_link_hash_defined
	  && bfd_is_abs_section (h->root.u.def.section))
	info->callbacks->einfo (_("%P: Possible gc problem: relocation references defined, absolute symbol '%s'\n"),
				h->root.root.string);
