...
proc muck_with_mounts {package} {
  ...
  global parameter relinfo errorInfo env
  redhat_get_relinfo
  ...
  set releasemount [file join $optredhatmount $relinfo(gnupro) ]
  set releasehostdir [file join $releasemount H-$relinfo(host) ]
  set mountcommand [file join $releasehostdir bin mount.exe ]
  if { ! [ file exists $mountcommand ] } {
    set mountcommand mount
  }
  set umountcommand [file join $releasehostdir bin umount.exe ]
  if { ! [ file exists $umountcommand ] } {
    set umountcommand umount
  }
  set mkpasswdcommand [file join $releasehostdir bin mkpasswd ]
  set mkgroupcommand [file join $releasehostdir bin mkgroup ]
  set newincdir [file join $installdir $relinfo(gnupro) H-$relinfo(host) $relinfo(target) sys-include ]
  progress 5

  set env(PATH) "$releasehostdir;$env(PATH)"
  ...
  # Update /usr/include 
  if { [ file executable $newincdir ] } {
    catch {eval exec {$umountcommand -f -u /usr/include}}
    catch {eval exec {$umountcommand -f -s /usr/include}}
    catch {eval exec {mkdir -p /usr/include}}

    set fid [open "| $mountcommand -s -f -b $newincdir /usr/include 2>$errlog"]
    if { $fid < 0 } {
      error "Failed to run $mountcommand -s -f -b $newincdir /usr/include: $errorInfo"
    }
  ...
  set fid [open "| $mountcommand -s -f -b $releasemount $releasemountpoint 2>$errlog"]
  if { $fid < 0 } {
    error "Failed to run $mountcommand -s -f -b $releasemount $releasemountpoint: $errorInfo"
  }
  gets $fid mountoutput
  close $fid
  ...
  catch { eval exec {$releasehostdir/bin/find $releasemountpoint -name \*.exe -o -name \*.dll -exec chmod a+x \{\} \;} }
  catch { eval exec {$releasehostdir/bin/find $releasemountpoint exec chmod a+r \{\} \;} }
...
proc install_nt_path {installdir} {
    ...
    set newpath "$installdir\\$relinfo(gnupro)\\H-$relinfo(host)\\bin"
    if { [ file exists "$newpath" ] } {
      ...
      if { [ string first $newpath $oldpath ] < 0 } {
        ...
        set mes1 "Your search path has been updated,\nbut will require a reboot to take effect."
        set res [ tk_messageBox -message "$mes1" -type ok -icon info ]           
      }
    }
...
proc install_w95_path {installdir} {
    ...
    set gppath "$installdir\\$relinfo(gnupro)\\H-$relinfo(host)\\bin"
...
proc redhat_fixincludes { booga } {
    global env relinfo parameter tcl_platform
    ...
    set env(PATH) "$parameter(installdir)/$relinfo(gnupro)/H-$relinfo(host)/bin;$env(PATH)"
    ...
    set old_pwd [pwd]
    set makesysrootlink 0
    if { $tcl_platform(platform) == "windows"} {
      set instHost "i686-pc-cygwin"
    } else {
      ...
      switch -glob -- $guessHost {
        ...
        "sparc*-sun-solaris2.9*" {
    	    set makesysrootlink 1
            set instHost "sparc-sun-solaris2.9"
        }
        "hppa*-hp-hpux10.*" {
    ...
    set CygInstDir $parameter(installdir)
    regsub {\\} $CygInstDir {/} CygInstDir

    set iToolsDir $CygInstDir/$relinfo(gnupro)/H-$relinfo(host)/libexec/gcc/$relinfo(target)/$relinfo(gccvn)/install-tools
    ...
    set FIXLOG $CygInstDir/$relinfo(gnupro)/H-$relinfo(host)/fixlog.[pid]
    set ERRLOG $CygInstDir/$relinfo(gnupro)/H-$relinfo(host)/errlog.[pid]
    set Ginc $CygInstDir/$relinfo(gnupro)/H-$relinfo(host)/lib/gcc/$relinfo(target)/$relinfo(gccvn)/include
    if { ! [file isdirectory $Ginc] } {
        error "Directory \"$Ginc\" does not exist.  Can not run fixincludes."
    }

    set env(CPP) "$CygInstDir/$relinfo(gnupro)/H-$relinfo(host)/bin/gcc -E"
    set env(GCC_EXEC_PREFIX) $CygInstDir/$relinfo(gnupro)/H-$relinfo(host)/libexec/gcc/
    ...
    if { $tcl_platform(platform) == "windows"} {
      set env(SHELL) $old_pwd/utils/H-windows/bin/sh.exe
    } else {
      set env(SHELL) /bin/sh
    }

    set env(MACRO_LIST) "/dev/null"
    ...
    if { [file exists $iToolsDir/fixinc.sh] } {
        cd $iToolsDir
	catch {eval exec $env(SHELL) $iToolsDir/fixinc.sh $Ginc $FixIncDir > $FIXLOG 2>$ERRLOG } ret
    }
        if { $ret != "" } {
            error "($ret)Running fixinc.sh failed.  Verbose output in \"$FIXLOG\".  Error output in \"$ERRLOG\"."
	}
	# System header files now ANSI compatible.
    if { [file exists $iToolsDir/fixproto] } {
        cd $iToolsDir
	catch {eval exec $env(SHELL) $iToolsDir/fixproto $Ginc $Ginc $FixIncDir >> $FIXLOG 2>>$ERRLOG} ret
        if { $ret != "" } {
...
proc resolve_cygwin_dll_version { rev }  {
  global env
  set needdll 1

  # Make sure sshd isn't keeping cygwin1.dll open
  catch { eval exec { net stop sshd } }

  foreach maybedir [split $env(PATH) ";"] {
    set tfile [file join $maybedir cygwin1.dll]
    if {! [string match {./utils/H-windows/bin/cygwin1.dll} $tfile]} {
      if {[file exists $tfile]} {
        set f [open $tfile "r"]
        fconfigure $f -translation binary
        set c [read $f]
        close $f
        regsub {.*%%% Cygwin dll major: ([0-9.]*)\n.*} $c {\1} majorver
        regsub {.*%%% Cygwin dll minor: ([0-9.]*)\n.*} $c {\1} minorver
	regsub {(1)0*([1-9][0-9]*)} $majorver {\1.\2} nmajorver
        set dllver $nmajorver.$minorver

        if [ string compare $rev $dllver ] {
          # We have a version mismatch, lets resolve it
          set mes1 "Warning: A conflicting cygwin1.dll has been found:\n\n$tfile is version $dllver (Must use $rev or later)\n\nRename to $tfile.old?"
          set resp [ tk_messageBox -message "$mes1" -type yesno -icon warning ]
          if [ string compare $resp "no" ] {
            catch { [ file rename $tfile $tfile.old ] }
            if {[file exists $tfile]} {
              set mes2 "Unable to rename $tfile\n\nPlease manually remove before proceeding."
              set res [ tk_messageBox -message "$mes2" -type ok -icon error ]
            }
          } else {
              set mes2 "Proceeding with conflicting DLL.\nThis may cause problems."
              set res [ tk_messageBox -message "$mes2" -type ok -icon info ]
          }
        } else {
          set needdll 0
        }

        if { [ info exists majorver ] && [ info exists minorver ] } {
          if { $minorver < 10 } {
            set minorver "00$minorver"
          } elseif { $minorver < 100 } {
            set minorver "0$minorver"
          }
