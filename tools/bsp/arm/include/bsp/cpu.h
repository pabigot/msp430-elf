...
#ifdef __XSCALE__
  static inline void __cpwait (void)
  {
      int i;
...
#define ARM_BTB_DISABLED                         0x00000000
#define ARM_BTB_ENABLED                          0x00000800
...
#ifdef __XSCALE__
#undef  ARM_MMU_SECTION
#define ARM_MMU_SECTION(ttb_base, actual_base, virtual_base, cacheable, bufferable, xbit, ecc, perm) \
    {                                                                                     \
        register union ARM_MMU_FIRST_LEVEL_DESCRIPTOR desc;                               \
                                                                                          \
        desc.word = 0;                                                                    \
...
#undef  X_ARM_MMU_SECTION
#define X_ARM_MMU_SECTION(abase,vbase,size,cache,buff,xbit,ecc,access)    \
    { int i; int j = abase; int k = vbase;                             \
      for (i = size; i > 0 ; i--,j++,k++)                              \
      {                                                                \
        ARM_MMU_SECTION(ttb_base, j, k, cache, buff, xbit, ecc, access);  \
      }                                                                \
    }
#endif
