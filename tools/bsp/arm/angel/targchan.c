 ...
 *
 *
 * targchan.c - Semi Synchronous Target side channel interface for Angel.
 * Derived from hostchan.c.
 */
#include <bsp/bsp.h>
#include <stdlib.h>
...
AdpErrs Adp_OpenDevice(const char *name, const char *arg, unsigned int heartbeat_on)
{
    int i;
    AdpErrs retc;
    ChannelID chan;
    ...
    const ChannelID chan, Packet *packet, AsyncMode mode)
{
    ...
    if (deviceToUse == NULL) {
#ifdef DEBUG
	printf("Adp_ChannelWrite: Device not open!\n");
#endif
        return adp_device_not_open;
    }
    ...
    if (invalidChannelID(chan)) {
#ifdef DEBUG
	printf("Adp_ChannelWrite: Bad channel!\n");
#endif
        return adp_bad_channel_id;
    }
...
#ifdef DEBUG
        printf("received a bad DC_DBUG packet\n");
#endif
	 ...
	 */
	    ...
	    *finished = TRUE;
        ...
        if (packet->pk_length < CF_DATA_BYTE_POS) {
            ...
            if ((packet->pk_buffer[CF_FLAGS_BYTE_POS]) & CF_RESEND) {
                ...
                if (HomeSeq != msg_oppo) {
                    ...
                    if (!found)
                        bsp_printf("trying to resend non-existent packets\n");
                }
                ...
                printf("received a duplicate\n"); /* FIXME */
                         ...
                         */
                printf("received out of sequence\n"); /* FIXME */
                        DevSW_FreePacket(packet);
                        send_resend_msg(DC_DBUG);
                    }
                    else
                                 ...
                                 bool *const finished  )
{
    ...
    {
#ifdef DEBUG
	printf("async_process_write: adp_write_busy\n");
#endif
...
static void async_process_callbacks( void )
{
    ...
    for ( i = 0; i < num_async_callbacks; ++i ) {
       async_callbacks[i]( deviceToUse, NULL );
    }
}
