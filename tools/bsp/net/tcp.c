/*
 * tcp.c -- Polling TCP server for Cygnus BSP
 *
...
tcp_send(tcp_socket_t *s, int flags, int resend)
{
	...
	tcp->hdr_len = 6;
	*(dword *)(tcp+1) = htonl(0x02040000 | MAX_TCP_DATA);
	s->data_bytes = 0;
    } else {
    ...
    if (!resend) {
	tcp->src_port = s->our_port;
	tcp->dest_port = s->his_port;
	tcp->flags = flags;
    ...
    tcp->checksum = __sum((word *)tcp, pkt->pkt_bytes, __pseudo_sum(ip));
...
handle_data(tcp_socket_t *s, pktbuf_t *pkt)
{
    ...
    BSPLOG(bsp_log("tcp data: seq[%u] len[%d].\n", seq, data_len));
...
__tcp_handler(pktbuf_t *pkt, ip_route_t *r)
{
	...
	for (prev = NULL, s = tcp_list; s; prev = s, s = s->next) {
	    if (s->our_port == tcp->dest_port) {
		if (s->his_port == 0)
		    break;
		if (s->his_port == tcp->src_port &&
...
{
    BSPLOG(bsp_log("tcp_listen: s[%x] port[%u]\n", s, port));
