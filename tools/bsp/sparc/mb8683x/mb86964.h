/*
 * mb86964.h -- Definitions for Fujitsu MB86964 NIC on Sparclite MB8683x EVB.
 *
...
#define mb_regs ((struct mb86964_regs *)0x20000002)


static inline unsigned short
mb86964_read(reg16 *p)
{
    unsigned short rval;

    asm volatile("lduha [%1] 4, %0\n" : "=r" (rval) : "r" (p));

    return rval;
}

static inline void
mb86964_write(reg16 *p, unsigned short val)
{
    asm volatile("stha %0, [%1] 4\n" : : "r" (val), "r" (p) );
}

static inline void
mb86964_write_config0(unsigned char val)
{
    reg8 *p = (reg8 *)0x2000000e;

    asm volatile("stba %0, [%1] 4\n" : : "r" (val), "r" (p) );
}
