...
#define FPSR_Z   0x4000000
#define FPSR_N   0x8000000
#define FPSR_NAN 0x1000000

#define MAKE_FPSR(fpsr,r) (fpsr) = (((fpsr) & 0x00FFFFFF)	\
    | ((r) == 0 ? FPSR_Z : 0) \
    | ((r) <  0 ? FPSR_N : 0) \
    | (__builtin_isunordered (r, 0.0) ? FPSR_NAN : 0))
...
{
    int N = (regs.fpsr & FPSR_N) != 0;
    int Z = (regs.fpsr & FPSR_Z) != 0;
    /* int I = (regs.fpsr & 0x2000000) != 0; */
    int NotANumber = (regs.fpsr & FPSR_NAN) != 0;
...
void fpp_opp (uae_u32 opcode, uae_u16 extra)
{
	...
	case 0x37:
	    ...
	    break;
	case 0x38:		/* FCMP */
	    MAKE_FPSR (regs.fpsr, regs.fp[reg] - src);
	    if (__builtin_isunordered (regs.fp[reg], src))
	      regs.fpsr |= FPSR_NAN;
	    else
	      {
		regs.fpsr &= ~ FPSR_NAN;
