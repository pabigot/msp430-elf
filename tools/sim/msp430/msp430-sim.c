...
static int
loader_write_mem (SIM_DESC sd, SIM_ADDR taddr, const unsigned char *buf, int bytes)
{
  SIM_CPU *cpu = sd->cpu;
	 ...
	 struct bfd *abfd, char **argv)
{
  ...
  CPU_PC_FETCH (sd->cpu) = msp430_pc_fetch;
  CPU_PC_STORE (sd->cpu) = msp430_pc_store;
  CPU_REG_FETCH (sd->cpu) = msp430_reg_fetch;
  CPU_REG_STORE (sd->cpu) = msp430_reg_store;

  /* Allocate memory if none specified by user.  */
  if (sim_core_read_buffer (sd, sd->cpu, read_map, &c, 0x200, 1) == 0)
    sim_do_commandf (sd, "memory-region 0,0x10000");
  if (sim_core_read_buffer (sd, sd->cpu, read_map, &c, 0xfffe, 1) == 0)
    sim_do_commandf (sd, "memory-region 0xfffe,2");
  if (sim_core_read_buffer (sd, sd->cpu, read_map, &c, 0x10000, 1) == 0)
  ...
  sd->cpu->state.cio_breakpoint = lookup_symbol (STATE_PROG_BFD (sd), "C$$IO$$");
  sd->cpu->state.cio_buffer = lookup_symbol (STATE_PROG_BFD (sd), "__CIOBUF__");
  if (sd->cpu->state.cio_buffer == -1)
    sd->cpu->state.cio_buffer = lookup_symbol (STATE_PROG_BFD (sd), "_CIOBUF_");
		    ...
		    char **env)
{
  ...
  c = sim_core_read_buffer (sd, sd->cpu, read_map, resetv, 0xfffe, 2);

  new_pc = resetv[0] + 256 * resetv[1];
  sim_pc_set (sd->cpu, new_pc);
  msp430_pc_store (sd->cpu, new_pc);
...
msp430_getbyte (void *vld)
{
  ...
  char buf[1];
  sim_core_read_buffer (ld->sd, ld->sd->cpu, read_map, buf, ld->gb_addr, 1);
  ld->gb_addr ++;
  return buf[0];
}

#define REG(N) sd->cpu->state.regs[(N)]
...
{
  if (TRACE_VPU_P (sd->cpu))
    trace_generic (sd, sd->cpu, TRACE_VPU_IDX,
...
get_op (SIM_DESC sd, MSP430_Opcode_Decoded *opc, int n)
{
  ...
  switch (op->type)
	...
	{
	case 8:
	  sim_core_read_buffer (sd, sd->cpu, read_map, buf, addr, 1);
	  rv = buf[0];
	  break;
	case 16:
	  sim_core_read_buffer (sd, sd->cpu, read_map, buf, addr, 2);
	  rv = buf[0] | (buf[1] << 8);
	  break;
	case 20:
	case 32:
	  sim_core_read_buffer (sd, sd->cpu, read_map, buf, addr, 4);
	...
	rv = 2;
#endif
      if (TRACE_MEMORY_P (sd->cpu))
	trace_generic (sd, sd->cpu, TRACE_MEMORY_IDX,
...
put_op (SIM_DESC sd, MSP430_Opcode_Decoded *opc, int n, int val)
{
  ...
  switch (op->type)
	...
	case 8:
	  buf[0] = val;
	  sim_core_write_buffer (sd, sd->cpu, write_map, buf, addr, 1);
	  break;
	case 16:
	  buf[0] = val;
	  buf[1] = val >> 8;
	  sim_core_write_buffer (sd, sd->cpu, write_map, buf, addr, 2);
	  break;
	case 20:
	case 32:
	  buf[0] = val;
	  ...
	  buf[2] = val >> 16;
	  buf[3] = val >> 24;
	  sim_core_write_buffer (sd, sd->cpu, write_map, buf, addr, 4);
	  break;
	default:
	  assert (! opc->size);
	  break;
...
#define CIO_L(n) (parms[(n)] + parms[(n)+1] * 256 + parms[(n)+2] * 65536 + parms[(n)+3] * 16777216)
...
msp430_cio (SIM_DESC sd)
{
  ...
  sim_core_read_buffer (sd, sd->cpu, 0, parms, sd->cpu->state.cio_buffer, 5);
  length = CIO_I (0);
  command = parms[2];

  sim_core_read_buffer (sd, sd->cpu, 0, parms, sd->cpu->state.cio_buffer+3, 8);
  sim_core_read_buffer (sd, sd->cpu, 0, buffer, sd->cpu->state.cio_buffer + 11, length);
  ...
  sim_core_write_buffer (sd, sd->cpu, 0, parms, sd->cpu->state.cio_buffer+4, 8);
  if (ret_buflen)
    sim_core_write_buffer (sd, sd->cpu, 0, buffer, sd->cpu->state.cio_buffer+12, ret_buflen);
}
		 ...
		 struct disassemble_info *dinfo)
{
  ...
  sim_core_read_buffer (sd, sd->cpu, 0, myaddr, memaddr, length);
  return 0;
}
...
#define DO_ALU(OP,SOP,MORE)						\
  {									\
    ...
    if (TRACE_ALU_P(sd->cpu))						\
      trace_generic (sd, sd->cpu, TRACE_ALU_IDX, "ALU: %#x %s %#x %s = %#x", s1, SOP, s2, #MORE, result); \
    DEST (result);							\
  }
	  ...
	  int overflow)
{
  ...
  if (TRACE_ALU_P (sd->cpu))
    {
      if (SR != new_f)
	trace_generic (sd, sd->cpu, TRACE_ALU_IDX, "FLAGS: %s -> %s", flags2string(SR), flags2string(new_f));
      else
	trace_generic (sd, sd->cpu, TRACE_ALU_IDX, "FLAGS: %s", flags2string(new_f));
    }
  SR = new_f;
}
...
/* These two assume unsigned 16-bit (four digit) words.  Mask off unwanted bits
   for byte operations. */
static int
bcd_to_binary (int v)
{
...
msp430_step_once (SIM_DESC sd)
{
  ...
  if (opcode_pc < 0x10)
    {
      ...
      sim_engine_halt (sd, sd->cpu, NULL,
		       sd->cpu->state.regs[0],
		       sim_exited, -1);
      return;
    }

  if (PC == sd->cpu->state.cio_breakpoint && STATE_OPEN_KIND (sd) != SIM_OPEN_DEBUG)
    msp430_cio (sd);

  ld.sd = sd;
  ld.gb_addr = PC;
  opsize = msp430_decode_opcode (sd->cpu->state.regs[0], opcode, msp430_getbyte, &ld);
  PC += opsize;
  if (opsize <= 0)
    {
  ...
  if (opcode->repeat_reg)
    n_repeats = (sd->cpu->state.regs[opcode->repeats] & 0x000f) + 1;
  else
...
#define CPU (sd->cpu)
  ...
  if (TRACE_ANY_P (CPU))
    trace_prefix (sd, CPU, NULL_CIA, opcode_pc, TRACE_LINENUM_P (CPU), NULL, 0, "");
  ...
  switch (opcode->id)
	...
	if (TRACE_ALU_P(sd->cpu))
	  trace_generic (sd, sd->cpu, TRACE_ALU_IDX,
			 "ADDC: %#x + %#x + %d = %#x",
	  ...
	  for (i=0; i<16; i++)
	    {
	      if (i % 4 == 0)
		fprintf (stderr, "\t");
	      fprintf (stderr, "R%-2d %05x   ", i, sd->cpu->state.regs[i]);
	      if (i % 4 == 3)
		{
		  int sp = SP + (3-(i/4))*2;
		  unsigned char buf[2];
		  sim_core_read_buffer (sd, sd->cpu, read_map, buf, sp, 2);
		  fprintf(stderr, "\tSP%+d: %04x", sp-SP, buf[0] + buf[1]*256);
		  if (i / 4 == 0)
		    {
		      int flags = SR;
      ...
      if ((u1 & ~0x3f) == 0x00180)
	{
	  ...
	  sc.func = syscall_num;
	  sc.arg1 = sd->cpu->state.regs[12];
	  sc.arg2 = sd->cpu->state.regs[13];
	  sc.arg3 = sd->cpu->state.regs[14];
	  sc.arg4 = sd->cpu->state.regs[15];

	  if (TRACE_SYSCALL_P(sd->cpu))
	    {
		...
		case TARGET_SYS_unlink:
		  syscall_name = "unlink(%#x)";
		  break;
		}
	      trace_generic (sd, sd->cpu, TRACE_SYSCALL_IDX,
			     syscall_name, sc.arg1, sc.arg2, sc.arg3, sc.arg4);
	    }

	  /* Handle SYS_exit here.  */
	  if (syscall_num == TARGET_SYS_exit)
	    {
	      sim_engine_halt (sd, sd->cpu, NULL,
			       sd->cpu->state.regs[0],
			       sim_exited, sc.arg1);
	  ...
	  sd->cpu->state.regs[12] = sc.result;
	  break;
	}
	...
	case MSC_true:
      ...
      if (u1)
	{
	  if (TRACE_BRANCH_P (sd->cpu))
	    trace_generic (sd, sd->cpu, TRACE_BRANCH_IDX,
