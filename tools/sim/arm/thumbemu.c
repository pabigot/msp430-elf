  ...
  * pvalid = t_undefined;
  ...
  switch (tBITS (5,12))
	...
	* pvalid = t_resolved;
	...
	ARMword result = state->Reg[Rn] ^ imm12;
		      ...
		      tdstate *     pvalid)
{
  ...
  switch (tinstr & 0xFFC0)
    ...
    case 0xb650: /* setend */
    default:  
      fprintf (stderr, " UNHANDLED v6 insn: %x\n", tinstr);
      * pvalid = t_undefined;
      return;
    }
  /* Indicate that the instruction has been processed.  */
		   ...
		   ARMword *     ainstr)
{
  ...
  if (trace)
    fprintf (stderr, "pc: %x, Thumb instr: %x [skip %x]", pc & ~1, tinstr, skipping_32bit_thumb);

  /* Debugging to catch non updates.  */
  *ainstr = 0xDEADC0DE;
  ...
  switch ((tinstr & 0xF800) >> 11)
	...
	* ainstr |= (1 << 20);
      ...
      if ((tinstr & (1 << 10)) == 0)
	{
	  ...
	  /* base */
	  *ainstr = subset[((tinstr & 0x03C0) >> 6) + (in_IT_block () ? 16 : 0)].opcode;
	...
	*ainstr = subset[(tinstr & 0x1800) >> 11]	/* base */
	...
	{
	  /* Format 16 */
	  int doit = test_cond (tBITS (8,11), state);
	  if (doit)
	    {
	      state->Reg[15] = (pc + 4
				+ (((tinstr & 0x7F) << 1)
