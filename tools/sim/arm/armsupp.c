...
handle_VFP_xfer (ARMul_State * state, ARMword instr)
{
  ...
  switch (BITS (20, 24))
    ...
    case 0x04:
    case 0x05:
      fprintf (stderr, "SIM: VFP: Unimplemented: 64-bit transfer\n");
      return;
	...
	ARMword address = base + (BIT (23) ? imm32 : - imm32);
	int dest;
      ...
      if (BITS (16, 19) == 13)
	{
	  ...
	  if (BIT (8))
	      ...
	      while (num--)
		{
		  if (state->bigendSig)
		    {
		      VFP_dword (dreg) = ARMul_LoadWordN (state, address);
		      VFP_dword (dreg) <<= 32;
		      VFP_dword (dreg) |= ARMul_LoadWordN (state, address + 4);
		    }
		  else
		    {
		      VFP_dword (dreg) = ARMul_LoadWordN (state, address + 4);
