...
dnl start-sanitize-redhat
AC_ARG_WITH(gmp-headers,
  AS_HELP_STRING([--with-gmp-headers=DIR],
                 [use a non-installed sources for GMP in DIR]),
  gmp_headers_dir=$with_gmp_headers
  if test -z "$with_libgmp_prefix"
  then
    CPPFLAGS="$CPPFLAGS -I$gmp_headers_dir -I$gmp_headers_dir/tune"
  else
    AC_MSG_ERROR([cannot use --with-gmp-headers and --with-gmp-prefix together])
  fi)

dnl end-sanitize-redhat
