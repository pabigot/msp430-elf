  ...
  while (start < end)
      ...
      if (is_eh ? (cie_id == 0) : (cie_id == DW_CIE_ID))
	{
	  ...
	  if (do_debug_frames_interp)
	    printf ("\n%0*lx %0*lx %0*lx CIE \"%s\" cf=%d df=%d ra=%d\n",
		    fc->ptr_size * 2,
		    (unsigned long)(saved_start - section_start),
		    fc->ptr_size * 2,
		    length,
      ...
      else
	{
	  ...
	  printf ("\n%0*lx %0*lx %0*lx FDE cie=%0*lx pc=",
		  fc->ptr_size * 2,
		  (unsigned long)(saved_start - section_start),
		  fc->ptr_size * 2,
		  length,
      ...
      while (start < block_end)
	{
	  ...
	  switch (op)
		...
		printf ("  DW_CFA_advance_loc: %d to %0*lx\n",
			opa * fc->code_factor,
			fc->ptr_size * 2,
		...
		printf ("  DW_CFA_advance_loc1: %ld to %0*lx\n",
			ofs * fc->code_factor,
			fc->ptr_size * 2,			
		...
		printf ("  DW_CFA_advance_loc2: %ld to %0*lx\n",
			ofs * fc->code_factor,
			fc->ptr_size * 2,
			fc->pc_begin + ofs * fc->code_factor);
	      if (0)
		...
		printf ("  DW_CFA_advance_loc4: %ld to %0*lx\n",
			ofs * fc->code_factor,
			fc->ptr_size * 2,			
			fc->pc_begin + ofs * fc->code_factor);
	      if (0)
		...
		printf ("  DW_CFA_MIPS_advance_loc8: %ld to %0*lx\n",
			ofs * fc->code_factor,
			fc->ptr_size * 2,
			fc->pc_begin + ofs * fc->code_factor);
	      if (0)
