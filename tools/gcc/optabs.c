		  ...
		  rtx *ptest, enum machine_mode *pmode)
{
  ...
  if (!SCALAR_FLOAT_MODE_P (mode))
	...
	/* Prevent an infinite loop if the target does not support the required comparison mode.  */
...
gen_add2_insn (rtx x, rtx y)
{
  ...
  /* gen_reload() expects this function to return NULL if the insn
     cannot be created.  This is important for the m32c build of libiberty.  */
