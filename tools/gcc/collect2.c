...
main (int argc, char **argv)
{
  ...
  if (ld_file_name == NULL && selected_linker == USE_PLUGIN_LD)
    ld_file_name = find_a_file (&cpath, ld_suffixes[USE_DEFAULT_LD]);

  /* If we still fail, then tell the user, if they are interested.  */
  if ((vflag || debug) && ld_file_name == NULL)
	...
	notice ("  %s\n", p->prefix);
      ...
      notice (" %s\n", REAL_LD_FILE_NAME);
#endif
      notice ("  %s\n", full_ld_suffixes[selected_linker]);
    }
...
#ifdef LD_RTINIT
#ifdef _AIX43
/* To support the -binitfini option in the gnu linker, the __rtinit
   symbol must be defined.  For more information on __rtinit, see
   the linker emulation script aix.em.  */

static void
__aix_ld_rtinit_print (FILE * stream, char * initname, char * fininame)
{
  int i;
  char * long_long_operator = aix64_flag ? ".llong" : ".long";
  ...
  fprintf (stream, "f1i:	%s %s\n", long_long_operator, initname);
  fprintf (stream, "	.long s1i - __rtinit\n");
  fprintf (stream, "	.long 0\n");
  fprintf (stream, "f2i:	%s 0\n", long_long_operator);
  fprintf (stream, "	.long 0\n");
  fprintf (stream, "	.long 0\n");
  fprintf (stream, "f1f:	%s %s\n", long_long_operator, fininame);
  fprintf (stream, "	.long s1f - __rtinit\n");
  fprintf (stream, "	.long 0\n");
  fprintf (stream, "f2f:	%s 0\n", long_long_operator);
  fprintf (stream, "	.long 0\n");
  fprintf (stream, "	.long 0\n");
  fprintf (stream, "	.align 3\n");
  fprintf (stream, "s1i:\n");
...
write_c_file_stat (FILE *stream, const char *name ATTRIBUTE_UNUSED)
{
  ...
  __aix_ld_rtinit_print (stream, initname, fininame);
#endif
#endif
}
