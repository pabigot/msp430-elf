                          ...
                          enum machine_mode op0_mode)
{
  ...
  switch (code)
    ...
    case SUBREG:
      ...
      /* amylaar: simplify_subreg woes.  */
      /* If we are asked to replace (reg:SI 67) with (reg:HI 66) in
	 (subreg:HI (reg:SI 67) 2)), we end up with an invalid
	 subreg and simplify_subreg would panic.  */
      if (SUBREG_BYTE (x) >= GET_MODE_SIZE (op0_mode))
	{
	  new_rtx = gen_rtx_CLOBBER (GET_MODE (x), const0_rtx);
	  break;
	}
