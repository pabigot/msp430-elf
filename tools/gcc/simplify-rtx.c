				...
				rtx op, enum machine_mode op_mode)
{
  ...
  if (CONST_INT_P (op)
	...
	case ZERO_EXTEND:
	  /* When zero-extending a CONST_INT, we need to know its
             original mode.  */
	  /* FIXME: Needed for building newlib for xStormy16.  */
	  if (op_mode == VOIDmode)
	    return 0;
  ...
  /* We can do some operations on integer CONST_DOUBLEs.  Also allow
	...
	case ZERO_EXTEND:
	  /* FIXME: Encountered when building libm for xstormy16.  */
	  if (op_mode == VOIDmode)
	    return immed_double_const (first.low, 0, mode);
	  gcc_assert (op_mode != VOIDmode);
