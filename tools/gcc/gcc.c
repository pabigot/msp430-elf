...
#endif
	  /* The dejagnu test harness can sometimes pass null
	     argument items to gcc.  Just ignore them.  */
	  if (*arg == 0)
	    continue;
...
#ifdef TARGET_USES_RESTART
  if (use_pipes)
    {
      error ("warning: -pipe is not supported on this target");
      use_pipes = 0;
    }
#endif
...
warranty; not even for MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.\n\n"),
  ...
  if (verbose_flag)
	...
	if (version_string[n] == ' ')
      ...
      if (target_system_root != NULL)
	fnotice (stderr, "TARGET_SYSTEM_ROOT: %s\n", target_system_root);
