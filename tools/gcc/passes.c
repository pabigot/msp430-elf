...
void
register_one_dump_file (struct opt_pass *pass, int requested_dump_number)
{
  ...
  id = dump_register (dot_name, flag_name, glob_name, flags, optgroup_flags, requested_dump_number);
  ...
  while (added_pass_nodes)
    {
      ...
      int requested_dump_number = 0;

      /* Request that the dump file for the pass uses a number that is
	 one less than the number of the pass that comes after it.  */
      if (added_pass_nodes->pass->next != NULL
	  && get_dump_file_info (added_pass_nodes->pass->next->static_pass_number) != NULL)
	requested_dump_number = get_dump_file_info (added_pass_nodes->pass->next->static_pass_number)->num - 1;
      register_one_dump_file (added_pass_nodes->pass, requested_dump_number);
