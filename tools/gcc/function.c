...
expand_function_end (void)
{
  ...
  if (! EXIT_IGNORE_STACK
    ...
    {
      rtx tem = 0, seq;
      /* We are inserting the stack_save insn at the start of the
	 function, but our current location is the end of the function.
	 Temporarily suppress the location information so that we do
	 not emit line debug information that confuses GDB.  */
      location_t sav = curr_insn_location ();
...
emit_return_into_block (bool simple_p, basic_block bb)
{
  ...
  if (GET_CODE (pat) == PARALLEL)
    pat = XVECEXP (pat, 0, 0);
  if (! ANY_RETURN_P (pat)
      && GET_CODE (PATTERN (jump)) == PARALLEL)
    {
      int i;

      /* The (return) inside a parallel insn does not
	 have to be the first element in the vector.  */
      for (i = 1; i < XVECLEN (PATTERN (jump), 0); i++)
	{
	  pat = XVECEXP (PATTERN (jump), 0, i);
	  if (ANY_RETURN_P (pat))
	    break;
	}
    }
