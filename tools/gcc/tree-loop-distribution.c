...
classify_partition (loop_p loop, struct graph *rdg, partition_t partition)
{
  ...
  if (single_store && !single_load)
	...
	return;
      /* Do not use the builtin memset code if we are compiling a function
	 called "memset".  The builtin code may decide to call a library
	 function called "memset" which would result in an infinite loop.  */
      if (current_function_name () != NULL
	  && strcmp (current_function_name (), "memset") == 0)
	return;
