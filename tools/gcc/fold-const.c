...
static int 
simple2_operand_p (tree exp, int level)
{
  /* Strip any conversions that don't change the machine mode.  */
  ...
  if (level >= BRANCH_COST (optimize_function_for_speed_p (cfun), false))
    return 0;
  ...
  switch (TREE_CODE (exp))
    ...
    /* XXX: it would be nice if we could determine whether or
       not these insns are fast and supported in this code.  */
    case PLUS_EXPR:
    case MINUS_EXPR:
    case BIT_AND_EXPR:
    case BIT_IOR_EXPR:
		    ...
		    tree lhs, tree rhs)
{
  ...
  /* ??? All the following redhat local code is experimental and the work
  ...
  if (optimize
      ...
      && lsize == rsize)
    {
      tree ltype = lang_hooks.types.type_for_size (lsize, (TYPE_UNSIGNED (ll_arg)
							   | TYPE_UNSIGNED (lr_arg)));
      tree rtype = lang_hooks.types.type_for_size (rsize, (TYPE_UNSIGNED (ll_arg)
							   | TYPE_UNSIGNED (lr_arg)));
      int common_size = (rsize > lsize) ? rsize : lsize;
      /* We make the result a signed.  */
      tree common_type = lang_hooks.types.type_for_size (common_size, 0);
	      ...
	      || (lcode == EQ_EXPR && code == TRUTH_AND_EXPR)))
	{
	  if (!integer_zerop (lr_arg))
	    ll_arg = build2 (BIT_XOR_EXPR, ltype, ll_arg, lr_arg);

	  if (!integer_zerop (rr_arg))
	    rl_arg = build2 (BIT_XOR_EXPR, rtype, rl_arg, rr_arg);

	  return build2 (lcode, truth_type,
			build2 (BIT_IOR_EXPR, common_type, ll_arg, rl_arg),
			integer_zero_node);
	}

      /* Here we insert some negates, or's, and and's,
	 so up the ante in terms of cost.  */
      if (BRANCH_COST (optimize_function_for_speed_p (cfun), false) >= 3)
	{
	  /* (a != 0 && b != 0)	=> (((a | -a) & (b | -b)) < 0)
	  ...
	  */
	  if (lcode == NE_EXPR && rcode == NE_EXPR && code == TRUTH_AND_EXPR)
		...
		rl_arg = build2 (BIT_XOR_EXPR, rtype, rl_arg, rr_arg);
	      ...
	      return build2 (LT_EXPR, truth_type,
			    build2 (BIT_AND_EXPR, common_type,
				   build2 (BIT_IOR_EXPR, common_type,
					  ll_arg,
      ...
      if (integer_zerop (lr_arg) && integer_zerop (rr_arg))
	{
	  ...
	  if (lcode == LT_EXPR && rcode == LT_EXPR)
	    {
	      /* Neither a, nor b is unsigned here
		 because of previous optimizations.  */
	      gcc_assert ((! TYPE_UNSIGNED (ll_arg)) && (! TYPE_UNSIGNED (rl_arg)));
	      
	      return build2 (lcode, truth_type,
			    build2 (((code == TRUTH_OR_EXPR)
				    ? BIT_IOR_EXPR
				    : BIT_AND_EXPR),
	  ...
	  if (BRANCH_COST (optimize_function_for_speed_p (cfun), false) >= 3)
	    {
		  ...
		  && code == TRUTH_AND_EXPR)
		{
		  /* b can not be unsigned here
		     because of previous optimizations.  */
		  gcc_assert (! TYPE_UNSIGNED (rl_arg));
		  ...
		  && code == TRUTH_AND_EXPR)
		{
		  ...
		  return build2 (LT_EXPR, truth_type,
				build2 (BIT_AND_EXPR, common_type,
				       build2 (BIT_IOR_EXPR, common_type,
					      ...
					      build1 (NEGATE_EXPR, common_type,
						      ll_arg)),
				       build1 (BIT_NOT_EXPR, common_type,
					       rl_arg)),
				integer_zero_node);
		}
		  ...
		  && code == TRUTH_AND_EXPR)
		{
		  /* a can not be unsigned here
		     because of previous optimizations.  */
		  gcc_assert (! TYPE_UNSIGNED (ll_arg));
