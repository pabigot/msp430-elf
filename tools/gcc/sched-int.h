...
#ifndef RCSP_SOFTWARE_PIPELINING
#define RCSP_SOFTWARE_PIPELINING 0
#endif

#if RCSP_SOFTWARE_PIPELINING
/* If the following variable value is non zero, the scheduler
   schedules the basic block insns in their original order.
   Currently, it is used to implement software pipelining.
   On the 1st insn scheduling, software pipelining is made for
   the loops.  On the 2nd insn scheduling, pipelined loop insns
   are scheduled in their original order to prevent changing
   the schedule. */
extern int only_evaluate_scheduled_block_p;

/* Functions in rcsp.c.  */
extern int  may_be_pipelined_p           (rtx, rtx);
extern void start_RCSP_interface         (rtx, rtx, int);
extern rtx  call_RCSP                    (FILE *);
extern void finish_RCSP_interface        (void);
#endif
