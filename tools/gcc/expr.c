		     ...
		     unsigned int expected_align, HOST_WIDE_INT expected_size)
{
  ...
  gcc_assert (method == BLOCK_OP_NORMAL || method == BLOCK_OP_TAILCALL || method == BLOCK_OP_BUILTIN);
	     ...
	     alias_set_type alias_set, bool nontemporal)
{
  ...
  if (mode == VOIDmode
      ...
      || (mode == BLKmode
	  && SLOW_UNALIGNED_ACCESS (mode, MEM_ALIGN (target))
	  && MEM_ALIGN (target) < TYPE_ALIGN (TREE_TYPE (exp)))
  ...
  if (TREE_CODE (exp) == COMPONENT_REF)
    {
      ...
      /* Issue 2743233 and others.  */
      /* Volatile bitfields should be accessed in the mode of the
	 field's type, not the mode computed based on the bit
	 size.  Check for this case first!  */
      if (TREE_THIS_VOLATILE (exp)
         && flag_strict_volatile_bitfields > 0
         && DECL_BIT_FIELD_TYPE (field))
	mode = TYPE_MODE (DECL_BIT_FIELD_TYPE (field));
      else
	/* FIXME: When removing the end--sanitize-redhat, also remove the TREE_THIS_VOLATILE case below.  */
      if (!DECL_BIT_FIELD (field))
	mode = DECL_MODE (field);
